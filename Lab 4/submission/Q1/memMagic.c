...
int init_node(struct node* n, unsigned size)
{
	...
	n->size = size;
	n->next = NULL;
	//n->data = mallo(sizeof(int) * size);
	n->data=mmap(n->data,sizeof(int)*size,PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE,0,0);
	assert(n->data > 0);
...
int inflate_node(struct node* n)
{
	...
	new_size = 2 * n->size;
	n->size = new_size;
	//n->data = ealloc(n->data, sizeof(int) * new_size);	
...
struct node *alloc_node()
{
	...
	n=mmap(n,sizeof(struct node),PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE,0,0);
	assert(n > 0);
